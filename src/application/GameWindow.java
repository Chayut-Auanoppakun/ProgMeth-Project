...
public class GameWindow {
	...
	private void updateMovement() {
		...
		double posy = 0;
		if (Main.getState() == 1) { // Server
			posx = GameServer.getServerX();
			posy = GameServer.getServerY();
		} else {
			posx = GameClient.getClientX();
			posy = GameClient.getClientY();
		}
		...
		if (Main.getState() == 1) { // Server
			GameServer.setPosition(posx, posy);
		} else if (Main.getState() == 2) { // Client
			GameClient.setPosition(posx, posy);
		}
	}
